...
def getSamples(dataFilename):
	...
	maxSamples = 0
	for subjectID in samplesPerSubject:
...
##	outline = 'Reference SampleID' + '\t' + 'Aligned SampleID' + '\t' + 'TaxonError' + '\t' + 'a' + '\t' + 'b' + '\t' + 'alpha' + '\t' + 'beta' + '\t' + 'overlap' + '\t' + 'Taxa Names' + '\t' + 'Alignment Scores(Pearson, Spearman, SSD, R-squared)' + '\n'
	outline = 'Reference SampleID' + '\t' + 'Aligned SampleID' + '\t' + 'TaxonError' + '\t' + 'a' + '\t' + 'b' + '\t' + 'alpha' + '\t' + 'beta' + '\t' + 'overlap' + '\t' + 'Taxa Names' + '\t' + 'Alignment Scores (SSD)' + '\n'
	outfile.writelines(outline)
	for subjectID  in samplesPerCycleInfoBySubject:
		samplesInfo = samplesPerCycleInfoBySubject[subjectID]
		for i in range(0, len(samplesInfo) - 1):
			sample1Size, sample1ID = samplesInfo[i]
			for j in range(i + 1, len(samplesInfo)):
...
class BSplinePlugin:
    def input(self, inputfile):
       ...
       self.txtfile = PyPluMA.prefix()+"/"+parameters["txtfile"]
       self.alignmentType = parameters["alignmentType"]
       self.useSplines = parameters["useSplines"]
    def run(self):
        pass
    def output(self, outfile):
        main(['',self.txtfile,self.alignmentType,self.useSplines,outfile])
